...
use oat_rust::algebra::chains::relative::{traits::SimplexDiameter, order::OrderOperatorSubComplexFiltrationSimplices};
...
impl IntoPy
    ...
    ForExport
        ...
        > 
{
    /// Returns a Padas data frame with columns `simplex` and `filtration`
    fn into_py(self, py: Python<'_>) -> Py<PyAny> {
        let dict = PyDict::new(py);
...
//  =====================================================================
//  ( Vec<SimplexFiltered>, OrderOperatorSubcomplexFiltrationSimplices )
//  =====================================================================

// for exporting the indices of the boundary oracle of a quotient chain complex 
...
{
    /// Returns a Padas data frame with columns `simplex`, `filtration`, `inSubspace` and `subspaceFiltration`.
    fn into_py(self, py: Python<'_>) -> Py<PyAny> {
        let dict = PyDict::new(py);
        dict.set_item( "simplex",     self.data.0.iter().map( |s| s.vertices() ).collect_vec() ).ok().unwrap();
        dict.set_item( "filtration",  self.data.0.iter().map( |s| s.filtration().into_inner() ).collect_vec() ).ok().unwrap();
        let subspace_diameter_function = | x: &SimplexFiltered<OrderedFloat<f64>> | self.data.1.diameter(x.vertices());
        dict.set_item( "inSubspace", self.data.0.iter().map( |s | subspace_diameter_function(s).is_some()).collect_vec() ).ok().unwrap();
        dict.set_item( "subspaceFiltration", self.data.0.iter().map( |s | subspace_diameter_function(s).unwrap_or(OrderedFloat(f64::INFINITY)).export()).collect_vec() ).ok().unwrap(); 
        let pandas = py.import("pandas").ok().unwrap();       
        pandas.call_method("DataFrame", ( dict, ), None)
            .map(Into::into).ok().unwrap()
    }
...
{
    /// Returns a Padas data frame with columns `simplex`, `filtration` and `inSubspace`. 
    fn to_object(&self, py: Python<'_>) -> PyObject {
        return self.clone().into_py(py);
    }
}
...
//  ================================================================================================
//  Vec<(SimplexFiltered<OrderedFloat<f64>>, Option<OrderedFloat<f64>>, Option<OrderedFloat<f64>>)>
//  ================================================================================================

// for exporting the indices of matched basis of a filtered quotient chain complex 
// NOTE: this is a vector of simplex keys and the associated birth/death of corresponding column views as generators for relative homology classes

impl IntoPy < Py<PyAny> > for 
    ForExport< Vec<(SimplexFiltered<OrderedFloat<f64>>, Option<OrderedFloat<f64>>, Option<OrderedFloat<f64>>)> >
{
    /// Returns a Padas data frame with columns `simplex`, `relativeCycleBirth`, `relativeBoundaryBirth`
    fn into_py(self, py: Python<'_>) -> Py<PyAny> {        let dict = PyDict::new(py);
        dict.set_item( "simplex", self.data.iter().map( |item| item.0.vertices() ).collect_vec() ).ok().unwrap();
        dict.set_item( "relativeCycleBirth", self.data.iter().map( |item| item.1.unwrap_or(OrderedFloat(f64::INFINITY)).into_inner() ).collect_vec() ).ok().unwrap();
        dict.set_item( "relativeBoiundaryBirth", self.data.iter().map( |item| item.2.unwrap_or(OrderedFloat(f64::INFINITY)).into_inner() ).collect_vec() ).ok().unwrap();
        let pandas = py.import("pandas").ok().unwrap();       
        pandas.call_method("DataFrame", ( dict, ), None)
            .map(Into::into).ok().unwrap()
    }
