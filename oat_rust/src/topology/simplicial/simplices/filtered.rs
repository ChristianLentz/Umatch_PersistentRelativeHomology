...
impl <FilVal: Clone + Debug>
    ...
    /// Wrap the vertices of the simplex as a string. 
    pub fn vertices_to_string(&self) -> String { 
        let vector_of_string_vertices: Vec<String> = self.vertices.iter().map(|vertex| vertex.to_string()).collect();
        vec![ "[".to_string(), vector_of_string_vertices.join(", "), "]".to_string() ].concat()
    }
}
